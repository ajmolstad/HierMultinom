library(MASS)
library(Rcpp)
sourceCpp("scripts/matMult.cpp")

# getGrad_Lik <- function(X, Y, beta){
#   t0 <- exp(crossprod(t(X), beta))
#   P <- t0/tcrossprod(colSums(t(t0)), rep(1, dim(beta)[2]))
#   grad <- crossprod(X, P - Y)/dim(Y)[1]
#   return(list("grad" = grad, "lik" = -sum(log(rowSums(Y*P)))/dim(Y)[1]))
# }
...
validationLik <- function(X, Xval, Yval, beta){
  ...
  if(any(x.sd.temp==0)){
    Xval.temp[,(which(x.sd.temp==0) + 1)] <- 0
  }
...
HierMultNestedOverlap.Path <- function(X, Y, groups, ngamma = 100, delta = 0.01, lambda.vec = 10^seq(-3, 0, length=10), tol = 1e-8, max.iter = 1e4, Xval, Yval){
  ...
  for(kk in 1:length(lambda.vec)){
    ...
    for(jj in 1:length(gamma.vec)){
      temp <- AccPGD_HierMultNestedOverlap(Xtrain, Y, groups, beta.init, lambda = lambda.vec[kk], gamma = gamma.vec[jj], max.iter = max.iter, tol = tol)
      #cat(sum(temp[-1,] == 0), "\n")
  ...
  for(kk in 1:length(lambda.vec)){
    beta.init <- beta.init0
    for(jj in 1:ngamma){
      temp <- AccPGD_HierMultNestedOverlap(Xtrain, Y, groups, beta.init, lambda = lambda.vec[kk], gamma = gamma.matrix[jj,kk], max.iter = max.iter, tol = tol)
