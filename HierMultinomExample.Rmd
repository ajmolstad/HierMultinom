...
\item $\texttt{delta}$: The ratio of minimum to maximum candidate $\gamma$ values (i.e., $\min_k \gamma_k =  \delta \max_k \gamma_k$ where $\gamma_k$ is the $k$th candidate tuning parameters). Check validation errors: if still decreasing at smallest $\gamma$ value, decrease $\texttt{delta}$ and rerun. 
